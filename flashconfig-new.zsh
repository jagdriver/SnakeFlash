...
# Version 2
# 2022-10-25 Niels Jørgen Nielsen
...
function ReadProperties() {
 ...
 # SnakeMonitor properties
   SNAKEMONITOR_VERSION=$SNAKEMONITOR_VERSION
   SNAKEMONITOR_LOG_FILE=$SNAKEMONITOR_LOG_FILE
...
function EditStacks()
{
   ...
   for f in $CURRENT_SWARM_DIR/Stacks/*; do
   echo "File name: $f"
...
function SetSwarmDirectories()
{
   CURRENT_WORK_DIR=$(pwd)
   TEMPLATE_DIR="$CURRENT_WORK_DIR/Artifacts/Templates"
   CURRENT_SWARM_DIR="$CURRENT_WORK_DIR/Swarm"
   #echo -e "Work dir: $CURRENT_WORK_DIR"
   #echo -e "Template dir: $TEMPLATE_DIR"
   #echo -e "Swarm dir: $CURRENT_SWARM_DIR"
}
...
function CreateNewSwarm()
{
   ...
   # But only on the manager node.
    ...
    LOOP=true
    while $LOOP; do
        read -e -p "Type swarm directory name > "
        if [[ -z "$REPLY" ]]; then
            echo -e -n "You must provide a Swarm directory name"
        else
            SWARM_NAME=$REPLY
            #echo -e -n "${SWARM_NAME}"
            LOOP=false
    ...
    ACTUAL_SWARM_DIR="$CURRENT_SWARM_DIR/$SWARM_NAME"
    CREATE_SWARM_DIR=true
    if [ -d "$ACTUAL_SWARM_DIR" ]; then
        CREATE_SWARM_DIR=false
        read -e -p "Directory exists, do you want to override it's content ? ${BLA}y | n > " -n 1 -r
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            OVERRIDE_SWARM_DIR=true
        fi
    fi

   #echo -e "\n Current swarm dir $ACTUAL_SWARM_DIR - $OVERRIDE_SWARM_DIR\n"
    
    if($CREATE_SWARM_DIR); then
        # Create swarm subdirectories
        cd Swarm
        mkdir $SWARM_NAME
        cd $SWARM_NAME
        mkdir Templates Config Stacks sn01 sn02 sn03 sn04
        #pwd

        #echo -e "\n Ls of swarm dir $ACTUAL_SWARM_DIR\n"
        #ls -l "$ACTUAL_SWARM_DIR"

        # Copy files to swarm subdirectory
        # Templates, Config, Stacks

        for f in ../../Artifacts/Templates/*; do
        cp -r "$f" "Templates"
        done
...
function EditSwarm()
{
   echo -e "Edit existing Swarm"
}

function DeleteSwarm()
{
   # Delete existing Swarm

   LOOP=true
   while $LOOP; do
      read -e -p "Type Swarm name > "
      if [[ -z "$REPLY" ]]; then
         echo -e -n "You must provide a Swarm name"
      else
         SWARM_NAME=$REPLY
         #echo -e -n "Directory name: ${SWARM_NAME}"

         if [ -d $CURRENT_SWARM_DIR/$SWARM_NAME ]; then

            read -e -p "Swarm exists, do you want to delete it ? ${BLA}y | n > " -n 1 -r
            if [[ $REPLY =~ ^[Yy]$ ]]; then
               rm -r $CURRENT_SWARM_DIR/$SWARM_NAME
               echo -e -n "Swarm deleted"
            fi
            LOOP=false
         else
            echo -e -n "Swarm does not exist"
            LOOP=false
         fi
      fi
   done 
...
function GetHelp()
{
   # This is no good, the online converter can't convert the tables properly.
...
function MainMenu() {
   ...
   while [ "$QUIT_MENU" != "QUIT" ]; do
      ...
      select fav in "${SCRIPT_MENU[@]}"; do
         case $fav in
         "Create_New_Swarm")
         # Ask for swarm name and create swarm directories
         echo "$fav"
         SetSwarmDirectories
         CreateNewSwarm
         break
